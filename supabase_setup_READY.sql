...
-- ============================================================================
-- STEP 1: Replace  below with your actual test user's UUID
...
-- ============================================================================
-- âš ï¸ REPLACE '' WITH YOUR ACTUAL TEST USER UUID BELOW! âš ï¸
-- ============================================================================

-- Insert test user into public.users
INSERT INTO public.users (id, email, full_name, phone, role)
VALUES ('', 'test@lesdauphins.ca', 'Jean Tremblay', '514-555-0123', 'owner')
...
VALUES (
  ...
  true,
  '{"street": "3535 avenue Papineau, Apt 201", "city": "MontrÃ©al", "province": "QC", "postal_code": "H2K 4J9", "country": "Canada"}'::jsonb,
  '{"name": "Marie Tremblay", "phone": "514-555-0124", "email": "marie@example.com", "relationship": "Ã‰pouse"}'::jsonb,
  '2020-01-15',
  true
...
UPDATE public.accounts_receivable SET paid_date = '2025-01-05', payment_method = 'PrÃ©lÃ¨vement automatique' WHERE invoice_number = 'INV-2025-000';
...
INSERT INTO public.service_requests (request_number, unit_id, requester_id, request_type, priority, description, location_details, status) VALUES
  ('SR-2025-001', '33333333-3333-3333-3333-333333333333', '', 'Plomberie', 'high', 'Fuite d''eau sous l''Ã©vier de la cuisine.', 'Cuisine, sous l''Ã©vier', 'assigned'),
  ('SR-2025-002', '33333333-3333-3333-3333-333333333333', '', 'Ã‰lectricitÃ©', 'low', 'Une prise Ã©lectrique dans la chambre ne fonctionne plus.', 'Chambre principale, mur nord', 'submitted'),
  ('SR-2024-150', '33333333-3333-3333-3333-333333333333', '', 'Chauffage', 'medium', 'Le thermostat ne rÃ©pond pas correctement.', 'Salon', 'completed')
ON CONFLICT (request_number) DO NOTHING;

UPDATE public.service_requests SET completed_date = '2024-12-15', resolution_notes = 'Thermostat remplacÃ©.', cost = 85.00, fund_charged = 'owner_responsibility' WHERE request_number = 'SR-2024-150';
...
INSERT INTO public.documents (category, title, file_url, file_type, file_size, visibility, uploaded_by) VALUES
  ('Financier', 'Ã‰tats financiers 2024', '/documents/financier/etats-2024.pdf', 'application/pdf', 2048576, 'all_owners', ''),
  ('Gouvernance', 'AGA 2024 - ProcÃ¨s-verbal', '/documents/gouvernance/pv-aga-2024.pdf', 'application/pdf', 1024000, 'all_owners', '')
ON CONFLICT DO NOTHING;

-- Create communications
INSERT INTO public.communications (communication_type, from_user_id, to_user_ids, subject, body) VALUES
  ('announcement', '', ARRAY['']::UUID[], 'AssemblÃ©e gÃ©nÃ©rale annuelle - 15 mars 2025', 'Chers copropriÃ©taires, vous Ãªtes invitÃ©s Ã  l''AGA le 15 mars 2025 Ã  19h00.'),
  ('announcement', '', ARRAY['']::UUID[], 'Travaux d''entretien prÃ©ventif - 10 mars 2025', 'Inspection des ascenseurs le 10 mars entre 9h00 et 17h00.')
ON CONFLICT DO NOTHING;

-- Done!
DO $$
BEGIN
  RAISE NOTICE 'âœ… Setup complete! Tables created and sample data inserted.';
  RAISE NOTICE 'âœ… User ID configured: ';
  RAISE NOTICE 'ðŸ“§ Login: test@lesdauphins.ca';
  RAISE NOTICE 'ðŸ  Unit: 201 (Tour Sud)';
  RAISE NOTICE 'ðŸ’° Invoices: 5 created (3 pending, 1 paid, 1 overdue)';
  RAISE NOTICE 'ðŸ”§ Service Requests: 3 created';
  RAISE NOTICE '';
  RAISE NOTICE 'ðŸš€ Ready to launch! Run: npm run dev';
END $$;
